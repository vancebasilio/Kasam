...
enum ActionDescriptor {
    case read, edit, more, flag, trash
    
    func title(forDisplayMode displayMode: ButtonDisplayMode) -> String? {
        guard displayMode != .imageOnly else { return nil }
        switch self {
            case .read: return "Read"
            case .edit: return "Unread"
    ...
    #if canImport(Combine)
        switch self {
            case .read, .edit: return UIColor.colorFour
            case .more:
                if #available(iOS 13.0, *) {
    ...
    #else
        switch self {
        case .read, .unread: return #colorLiteral(red: 0.8980392157, green: 0.7333333333, blue: 0.3450980392, alpha: 1)
