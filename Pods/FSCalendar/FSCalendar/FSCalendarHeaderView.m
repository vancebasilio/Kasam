...
//  Modifed by Vance Basilio on 25/5/19.
//
...
- (void)layoutSubviews
{
    ...
    //Changes made to the below:
    self.collectionView.frame = CGRectMake(-self.fs_width/3 + 26, self.fs_height*0.1, self.fs_width * 1.25, self.fs_height*0.9);
}
...
- (void)layoutSubviews
{
    ...
    if (self.header.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        ...
        if (self.header.scrollEnabled) {
            //changes made to below (1.5 instead of 1.0)
            self.contentView.alpha = 1.0 - (1.5-self.header.calendar.appearance.headerMinimumDissolvedAlpha)*ABS(center-position)/self.fs_width;
        } else {
...
- (void)prepareLayout
{
    ...
    self.itemSize = CGSizeMake(self.collectionView.fs_width*((self.scrollDirection==UICollectionViewScrollDirectionHorizontal)?0.5:1), self.collectionView.fs_height
    );
